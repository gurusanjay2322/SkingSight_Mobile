...
const ResultsScreen = () => {
  ...
  const viewModel = useMemo(() => {
    ...
    return {
      ...
      weather: {
        temp: weather.current_temp || 0,
        humidity: weather.humidity || 0,
        uv: weather.uv_index || 0,
        aqi: weather.aqi || 0,
        location: weather.city || "Current Location",
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('beforeRemove', (e: any) => {
      // If we already saved, or if we are explicitly navigating to Home, allow it
      if (hasSaved || e.data.action.type === 'NAVIGATE' && e.data.action.payload?.name === 'Home') {
        return;
      }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => setImageModalVisible(true)} style={styles.backButton}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroCard}>
          <View style={styles.heroTop}>
            ...
            <View style={styles.heroRiskContainer}>
              ...
              <View style={[styles.heroBadge, { backgroundColor: RISK_COLORS[viewModel.riskLevel] || RISK_COLORS.Unknown }]}>
          ...
          {viewModel.insights && viewModel.insights.length > 0 && (
            <View style={styles.heroInsight}>
              ...
              <Text style={styles.heroInsightText}>{viewModel.insights[0]}</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.timingCard}>
          ...
          <Text style={styles.timingCurrent}>{viewModel.timing.current_advice}</Text>
          <View style={styles.timingRow}>
            <View style={styles.timingCol}>
              <Text style={styles.timingColTitle}>Recommended</Text>
              {viewModel.timing.recommended?.map((item: string, idx: number) => (
                <Text key={idx} style={styles.timingItem}>✅ {item}</Text>
              ))}
            </View>
            <View style={styles.timingCol}>
              <Text style={styles.timingColTitle}>Caution</Text>
              {viewModel.timing.avoid?.map((item: string, idx: number) => (
                <Text key={idx} style={[styles.timingItem, { color: "#991B1B" }]}>⚠️ {item}</Text>
              ))}
            </View>
          </View>
        </View>
        ...
        <View style={{ marginBottom: 32 }}>
          ...
          <View style={styles.weatherGrid}>
            ...
            <View style={[styles.weatherGridItem, { borderRightWidth: 0 }]}>
              ...
              <Text style={styles.weatherGridValue}>{viewModel.weather.aqi}</Text>
              <Text style={styles.weatherGridLabel}>AQI</Text>
            </View>
          </View>
          <View style={styles.locationRow}>
            <Ionicons name="location-outline" size={14} color="#71717A" />
            <Text style={styles.locationText}>{viewModel.weather.location}</Text>
          </View>
        </View>

        {/* Routine Section */}
        <View style={{ marginBottom: 24 }}>
          <Text style={[styles.heroLabel, { marginBottom: 4 }]}>AI-Generated Routine</Text>
          <Text style={styles.routineSubtitle}>Optimized for {viewModel.predictedClass} skin in {viewModel.weather.condition} weather.</Text>
          <View style={styles.routineSection}>
            <View style={styles.routineHeader}>
              <Ionicons name="sunny" size={20} color="#F59E0B" />
              <Text style={styles.routineTitle}>Morning</Text>
            </View>
            {viewModel.routine.morning?.map((item: RoutineStep, idx: number) => (
          ...
          <View style={styles.routineSection}>
            ...
            {viewModel.routine.evening?.map((item: RoutineStep, idx: number) => (
              <View key={idx} style={styles.routineItem}>
                <Text style={styles.routineStep}>{item.step}</Text>
                <Text style={styles.routineReason}>{item.reason}</Text>
              </View>
        ...
        {viewModel.suggestions && viewModel.suggestions.length > 0 && (
          <View style={{ marginBottom: 32 }}>
            <Text style={[styles.heroLabel, { marginBottom: 16 }]}>Personalized Tips</Text>
            {viewModel.suggestions.map((tip: string, idx: number) => (
              <View key={idx} style={styles.suggestionItem}>
                <Ionicons name="sparkles" size={16} color="#09090B" style={{ marginTop: 2 }} />
        ...
        {Object.keys(viewModel.classScores).length > 0 && (
          <View style={{ marginTop: 8 }}>
            <Text style={[styles.heroLabel, { marginBottom: 12 }]}>Analysis Confidence</Text>
            {Object.entries(viewModel.classScores).map(([label, score]: [string, any]) => (
              <View key={label} style={styles.chartRow}>
                <Text style={[styles.chartLabel, label.toLowerCase() === viewModel.predictedClass.toLowerCase() && styles.chartLabelActive]}>
                ...
                <View style={styles.chartBarBg}>
                  <View 
                    style={[
                      ...
                      { 
                        width: `${score}%`, 
                        backgroundColor: label.toLowerCase() === viewModel.predictedClass.toLowerCase() ? "#09090B" : "#E4E4E7" 
                      }
                    ]} 
                  />
                </View>
                <Text style={[styles.chartValue, label.toLowerCase() === viewModel.predictedClass.toLowerCase() && styles.chartValueActive]}>
                  {Math.round(score)}%
                </Text>
              </View>
            ))}
        ...
        {viewModel.disease && (
          <View style={{ marginTop: 32 }}>
            <Text style={[styles.heroLabel, { marginBottom: 16 }]}>Secondary Observations</Text>
            <View style={styles.diseaseBox}>
              <View style={styles.diseaseHeader}>
                <Ionicons name="medical-outline" size={20} color="#09090B" />
                <Text style={styles.diseaseTitle}>{viewModel.disease.label}</Text>
              </View>
              <Text style={styles.diseaseConfidence}>Confidence: {viewModel.disease.percentage}%</Text>
              <View style={styles.warningBox}>
                <Ionicons name="alert-circle" size={16} color="#991B1B" />
                <Text style={styles.warningText}>AI scans are not diagnoses. Consult a professional.</Text>
              </View>
            </View>
          </View>
        )}
        ...
        <View style={styles.sourcesCard}>
          ...
          <View style={styles.sourceItem}>
            ...
            <Text style={styles.sourceText}>Weather: OpenWeatherMap & Open-Meteo</Text>
          </View>
          <View style={styles.sourceItem}>
            <Ionicons name="hardware-chip-outline" size={14} color="#71717A" />
            <Text style={styles.sourceText}>Intelligence: SkinSight ML + GPT-3.5</Text>
          </View>
      ...
      <View style={styles.footer}>
        <View style={styles.actionRow}>
          <TouchableOpacity onPress={() => setChatVisible(true)} style={styles.outlineButton}>
      ...
      <ChatModal visible={chatVisible} onClose={() => setChatVisible(false)} context={viewModel.raw} />
      ...
      <CustomAlert
        ...
        type="warning"
        actions={[
          { text: "Go to Home", onPress: () => navigation.navigate("Home"), style: "destructive" },
          { text: "Cancel", style: "cancel" }
        ]}
      />
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  heroTop: { flexDirection: "column", alignItems: "flex-start", gap: 8, marginBottom: 16 },
  heroLabel: { fontSize: 13, color: "#71717A", fontWeight: "500", textTransform: "uppercase", letterSpacing: 0.5 },
  heroValue: { fontSize: 44, fontWeight: "700", color: "#09090B", letterSpacing: -1.5, textTransform: "capitalize" },
  heroRiskContainer: { marginTop: 8 },
  heroRiskLabel: { fontSize: 12, color: "#71717A", fontWeight: "500", marginBottom: 4 },
  heroBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 4 },
  heroBadgeText: { color: "#FFF", fontSize: 12, fontWeight: "600" },
  heroInsight: { flexDirection: "row", alignItems: "center", backgroundColor: "#FAFAFA", padding: 14, borderRadius: 8, gap: 10, borderWidth: 1, borderColor: "#E4E4E7" },
  heroInsightText: { flex: 1, fontSize: 14, color: "#27272A", lineHeight: 20 },
  timingCard: { backgroundColor: "#FFFFFF", borderRadius: 8, padding: 20, marginBottom: 16, borderWidth: 1, borderColor: "#E4E4E7" },
  timingHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 },
  timingTitle: { fontSize: 14, fontWeight: "600", color: "#09090B" },
  timingCurrent: { fontSize: 13, fontWeight: "500", marginBottom: 16, padding: 12, backgroundColor: "#FAFAFA", borderRadius: 6, color: "#3F3F46" },
  timingRow: { flexDirection: "row", gap: 16 },
  timingCol: { flex: 1 },
  timingColTitle: { fontSize: 12, fontWeight: "600", color: "#71717A", marginBottom: 8, textTransform: "uppercase" },
  timingItem: { fontSize: 13, color: "#27272A", marginBottom: 6, lineHeight: 18 },
  ...
  chartBarBg: { flex: 1, height: 8, backgroundColor: "#F4F4F5", borderRadius: 4, overflow: "hidden" },
  ...
  weatherGrid: { flexDirection: "row", flexWrap: "wrap", marginBottom: 16, borderWidth: 1, borderColor: "#E4E4E7", borderRadius: 8 },
  weatherGridItem: { width: "25%", alignItems: "center", paddingVertical: 16, borderRightWidth: 1, borderRightColor: "#F4F4F5" },
  weatherGridValue: { fontSize: 16, fontWeight: "700", color: "#09090B", marginTop: 4 },
  weatherGridLabel: { fontSize: 11, color: "#71717A", marginTop: 2 },
  locationRow: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 12, paddingVertical: 12, borderTopWidth: 1, borderTopColor: "#F4F4F5" },
  locationText: { fontSize: 13, color: "#71717A" },
  diseaseBox: { borderWidth: 1, borderRadius: 8, padding: 16, backgroundColor: "#FFFFFF", borderColor: "#E4E4E7" },
  diseaseHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 },
  diseaseTitle: { fontSize: 15, fontWeight: "600", color: "#09090B" },
  diseaseConfidence: { fontSize: 13, color: "#71717A", marginLeft: 28 },
  warningBox: { flexDirection: "row", alignItems: "center", backgroundColor: "#FEF2F2", padding: 10, borderRadius: 6, gap: 8, marginTop: 12 },
  ...
  routineHeader: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 12 },
  routineTitle: { fontSize: 15, fontWeight: "600", color: "#09090B" },
  routineItem: { marginBottom: 16, paddingLeft: 12, borderLeftWidth: 1, borderLeftColor: "#E4E4E7" },
  routineStep: { fontSize: 14, fontWeight: "600", color: "#27272A", marginBottom: 4 },
  routineReason: { fontSize: 13, color: "#71717A", lineHeight: 18 },
  sourcesCard: { padding: 16, borderTopWidth: 1, borderTopColor: "#E4E4E7", marginTop: 16 },
  sourcesHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 },
  sourcesTitle: { fontSize: 13, fontWeight: "600", color: "#71717A", textTransform: "uppercase" },
  sourceItem: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 },
  sourceText: { fontSize: 12, color: "#71717A" },
  suggestionItem: { flexDirection: "row", gap: 12, marginBottom: 12, paddingRight: 16 },
  suggestionText: { fontSize: 13, color: "#27272A", lineHeight: 20, flex: 1 },
  disclaimer: { flexDirection: "row", alignItems: "flex-start", backgroundColor: "#FAFAFA", padding: 12, borderRadius: 6, gap: 8, marginTop: 8 },
  ...
  outlineButton: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 12, borderRadius: 8, borderWidth: 1, borderColor: "#E4E4E7", backgroundColor: "#FFFFFF", gap: 8 },
  outlineButtonText: { fontSize: 14, fontWeight: "500", color: "#09090B" },
  secondaryButton: { width: "100%", alignItems: "center", justifyContent: "center", paddingVertical: 16, borderRadius: 8, backgroundColor: "#09090B" },
